...
from sidserver.i18n import _LE, _LW, _LI
...
def _convert_to_integers(id_value):
    ...
    # sidserver_user_id and sidserver_group_id are not integers. Let's
...
def create_key_directory(sidserver_user_id=None, sidserver_group_id=None):
    ...
    if not os.access(CONF.fernet_tokens.key_repository, os.F_OK):
        ...
        if sidserver_user_id and sidserver_group_id:
            os.chown(
                CONF.fernet_tokens.key_repository,
                sidserver_user_id,
                sidserver_group_id)
        elif sidserver_user_id or sidserver_group_id:
            LOG.warning(_LW(
                'Unable to change the ownership of [fernet_tokens] '
                'key_repository without a sidserver user ID and sidserver group '
                'ID both being provided: %s') %
                CONF.fernet_tokens.key_repository)


def _create_new_key(sidserver_user_id, sidserver_group_id):
    ...
    if sidserver_user_id and sidserver_group_id:
        old_egid = os.getegid()
        old_euid = os.geteuid()
        os.setegid(sidserver_group_id)
        os.seteuid(sidserver_user_id)
    elif sidserver_user_id or sidserver_group_id:
        LOG.warning(_LW(
            'Unable to change the ownership of the new key without a sidserver '
            'user ID and sidserver group ID both being provided: %s') %
            CONF.fernet_tokens.key_repository)
...
def initialize_key_repository(sidserver_user_id=None, sidserver_group_id=None):
    """Create a key repository and bootstrap it with a key.

    :param sidserver_user_id: User ID of the Keystone user.
    :param sidserver_group_id: Group ID of the Keystone user.
    ...
    _create_new_key(sidserver_user_id, sidserver_group_id)

    # ensure that we end up with a primary and secondary key
    rotate_keys(sidserver_user_id, sidserver_group_id)


def rotate_keys(sidserver_user_id=None, sidserver_group_id=None):
