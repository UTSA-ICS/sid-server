...
_CERTFILE = '/etc/sidserver/ssl/certs/signing_cert.pem'
_KEYFILE = '/etc/sidserver/ssl/private/signing_key.pem'
_SSO_CALLBACK = '/etc/sidserver/sso_callback_template.html'
...
FILE_OPTIONS = {
    ...
    'identity': [
        cfg.StrOpt('default_domain_id', default='default',
                   help='This references the domain to use for all '
                        ...
                        'for you by sidserver-manage db_sync in migration '
        ...
        cfg.StrOpt('driver',
                   default=('sidserver.identity.backends'
                            '.sql.Identity'),
    ...
    'identity_mapping': [
        cfg.StrOpt('driver',
                   default=('sidserver.identity.mapping_backends'
                            '.sql.Mapping'),
                   help='Keystone Identity Mapping backend driver.'),
        cfg.StrOpt('generator',
                   default=('sidserver.identity.id_generators'
                            '.sha256.Generator'),
        ...
        cfg.BoolOpt('backward_compatible_ids',
                    ...
                    help='The format of user and group IDs changed '
                         ...
                         '(e.g. LDAP), with sidserver providing a hash mapping '
    ...
    'trust': [
        ...
        cfg.StrOpt('driver',
                   default='sidserver.trust.backends.sql.Trust',
    ...
    'fernet_tokens': [
        cfg.StrOpt('key_repository',
                   default='/etc/sidserver/fernet-keys/',
        ...
        cfg.IntOpt('max_active_keys',
                   ...
                   help='This controls how many keys are held in rotation by '
                        'sidserver-manage fernet_rotate before they are '
                        'discarded. The default value of 3 means that '
                        'sidserver will maintain one staged key, one primary '
    ...
    'token': [
        ...
        cfg.IntOpt('expiration', default=3600,
                   help='Amount of time a token should remain valid '
                        '(in seconds).'),
        cfg.StrOpt('provider',
                   default='sidserver.token.providers.uuid.Provider',
                   help='Controls the token construction, validation, and '
                        'revocation operations. Core providers are '
                        '"sidserver.token.providers.[fernet|pkiz|pki|uuid].'
                        'Provider".'),
        cfg.StrOpt('driver',
                   default='sidserver.token.persistence.backends.sql.Token',
    ...
    'revoke': [
        cfg.StrOpt('driver',
                   default='sidserver.contrib.revoke.backends.sql.Revoke',
    ...
    'cache': [
        cfg.StrOpt('config_prefix', default='cache.sidserver',
        ...
        # unintentionally, we register a no-op as the sidserver default caching
        # backend.
        cfg.StrOpt('backend', default='sidserver.common.cache.noop',
                   help='Dogpile.cache backend module. It is recommended '
                        'that Memcache with pooling '
                        '(sidserver.cache.memcache_pool) or Redis '
        ...
        cfg.ListOpt('memcache_servers', default=['localhost:11211'],
                    ...
                    ' (dogpile.cache.memcache and sidserver.cache.memcache_pool'
                    ' backends only).'),
        cfg.IntOpt('memcache_dead_retry',
                   ...
                   ' sidserver.cache.memcache_pool backends only).'),
        ...
        cfg.IntOpt('memcache_pool_maxsize',
                   ...
                   ' memcached server. (sidserver.cache.memcache_pool backend'
                   ' only).'),
        cfg.IntOpt('memcache_pool_unused_timeout',
                   ...
                   ' (sidserver.cache.memcache_pool backend only).'),
    ...
    'ssl': [
        cfg.StrOpt('ca_key',
                   default='/etc/sidserver/ssl/private/cakey.pem',
    ...
    'signing': [
        cfg.StrOpt('certfile',
                   ...
                   help='Path of the certfile for token signing. For '
                        ...
                        'in using `sidserver-manage pki_setup` to generate '
                        'self-signed certificates.'),
        cfg.StrOpt('keyfile',
                   default=_KEYFILE,
                   help='Path of the keyfile for token signing.'),
        cfg.StrOpt('ca_certs',
                   default='/etc/sidserver/ssl/certs/ca.pem',
    ...
    'domain_config': [
        cfg.StrOpt('driver',
                   default='sidserver.resource.config_backends.sql.'
                           'DomainConfig',
    ...
    'credential': [
        cfg.StrOpt('driver',
                   default=('sidserver.credential.backends'
                            '.sql.Credential'),
                   help='Credential backend driver.'),
    ],
    'oauth1': [
        cfg.StrOpt('driver',
                   default='sidserver.contrib.oauth1.backends.sql.OAuth1',
                   help='Credential backend driver.'),
    ...
    'federation': [
        cfg.StrOpt('driver',
                   default='sidserver.contrib.federation.'
    ...
    'policy': [
        cfg.StrOpt('driver',
                   default='sidserver.policy.backends.sql.Policy',
    ...
    'endpoint_filter': [
        cfg.StrOpt('driver',
                   default='sidserver.contrib.endpoint_filter.backends'
                           '.sql.EndpointFilter',
    ...
    'endpoint_policy': [
        cfg.StrOpt('driver',
                   default='sidserver.contrib.endpoint_policy.backends'
                           '.sql.EndpointPolicy',
   ...
   'auth': [
        ...
        cfg.StrOpt('password',
                   default='sidserver.auth.plugins.password.Password',
                   help='The password auth plugin module.'),
        cfg.StrOpt('token',
                   default='sidserver.auth.plugins.token.Token',
                   help='The token auth plugin module.'),
        # deals with REMOTE_USER authentication
        cfg.StrOpt('external',
                   default='sidserver.auth.plugins.external.DefaultDomain',
                   help='The external (REMOTE_USER) auth plugin module.'),
        cfg.StrOpt('oauth1',
                   default='sidserver.auth.plugins.oauth1.OAuth',
                   help='The oAuth1.0 auth plugin module.'),
    ],
    'paste_deploy': [
        cfg.StrOpt('config_file', default='sidserver-paste.ini',
    ...
    'catalog': [
        ...
        cfg.StrOpt('driver',
                   default='sidserver.catalog.backends.sql.Catalog',
    ...
    'kvs': [
        ...
        cfg.StrOpt('config_prefix', default='sidserver.kvs',
                   help='Prefix for building the configuration dictionary '
    ...
    'saml': [
        ...
        cfg.StrOpt('idp_entity_id',
                   help='Entity ID value for unique Identity Provider '
                        ...
                        'For example: https://sidserver.example.com/v3/'
        ...
        cfg.StrOpt('idp_metadata_path',
                   default='/etc/sidserver/saml2_idp_metadata.xml',
                   help='Path to the Identity Provider Metadata file. '
                        'This file should be generated with the '
                        'sidserver-manage saml_idp_metadata command.'),
    ...
    'eventlet_server_ssl': [
        cfg.BoolOpt('enable', default=False, deprecated_name='enable',
                    ...
                    help='Toggle for SSL support on the Keystone '
                         'eventlet servers.'),
        cfg.StrOpt('certfile',
                   default="/etc/sidserver/ssl/certs/sidserver.pem",
                   ...
                   help='Path of the certfile for SSL. For non-production '
                        ...
                        '`sidserver-manage ssl_setup` to generate self-signed '
                        'certificates.'),
        cfg.StrOpt('keyfile',
                   default='/etc/sidserver/ssl/private/sidserverkey.pem',
...
CONF = cfg.CONF
oslo_messaging.set_transport_defaults(control_exchange='sidserver')
...
def list_opts():
    ...
    the "FILE_OPTIONS" in sidserver.common.config. This list will not include
    ...
    under the 'sidserver.config.opts' namespace.
