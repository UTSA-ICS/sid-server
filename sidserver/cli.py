...
from sidserver.common import openssl
...
class BasePermissionsSetup(BaseApp):
    ...
    def add_argument_parser(cls, subparsers):
        ...
        parser.add_argument('--sidserver-user', required=running_as_root)
        parser.add_argument('--sidserver-group', required=running_as_root)
        return parser

    @staticmethod
    def get_user_group():
        sidserver_user_id = None
        sidserver_group_id = None

        try:
            a = CONF.command.sidserver_user
            if a:
                sidserver_user_id = utils.get_unix_user(a)[0]
        except KeyError:
            raise ValueError("Unknown user '%s' in --sidserver-user" % a)

        try:
            a = CONF.command.sidserver_group
            if a:
                sidserver_group_id = utils.get_unix_group(a)[0]
        except KeyError:
            raise ValueError("Unknown group '%s' in --sidserver-group" % a)

        return sidserver_user_id, sidserver_group_id
...
class PKISetup(BaseCertificateSetup):
    ...
    @classmethod
    def main(cls):
        LOG.warn(_LW('sidserver-manage pki_setup is not recommended for '
                     'production use.'))
        sidserver_user_id, sidserver_group_id = cls.get_user_group()
        conf_pki = openssl.ConfigurePKI(sidserver_user_id, sidserver_group_id,
...
class SSLSetup(BaseCertificateSetup):
    ...
    @classmethod
    def main(cls):
        LOG.warn(_LW('sidserver-manage ssl_setup is not recommended for '
                     'production use.'))
        sidserver_user_id, sidserver_group_id = cls.get_user_group()
        conf_ssl = openssl.ConfigureSSL(sidserver_user_id, sidserver_group_id,
                                        rebuild=CONF.command.rebuild)
        conf_ssl.run()
...
class FernetSetup(BasePermissionsSetup):
    ...
    keys (using sidserver-manage fernet_rotate, for example).
    ...
    @classmethod
    def main(cls):
        from sidserver.token.providers.fernet import utils as fernet

        sidserver_user_id, sidserver_group_id = cls.get_user_group()
        fernet.create_key_directory(sidserver_user_id, sidserver_group_id)
        if fernet.validate_key_repository():
            fernet.initialize_key_repository(
                sidserver_user_id, sidserver_group_id)


class FernetRotate(BasePermissionsSetup):
    """Rotate Fernet encryption keys.

    This assumes you have already run sidserver-manage fernet_setup.
    ...
    def main(cls):
        ...
        sidserver_user_id, sidserver_group_id = cls.get_user_group()
        if fernet.validate_key_repository():
            fernet.rotate_keys(sidserver_user_id, sidserver_group_id)
...
class DomainConfigUploadFiles(object):
    ...
    def upload_configs_to_database(self, file_name, domain_name):
        ...
        try:
            ...
            print(_('To get a more detailed information on this error, re-run '
                    ...
                    'sidserver-manage domain_config_upload --domain-name %s') %
                  domain_name)
            pass
    ...
    def read_domain_configs_from_files(self):
        ...
        # filename pattern of 'sidserver.<domain_name>.conf'
    ...
    def run(self):
        ...
        try:
            ...
            # related to set up
            print(_('Unable to access the sidserver database, please check it '
...
class SamlIdentityProviderMetadata(BaseApp):
    ...
    def main():
        ...
        from sidserver.contrib.federation import idp
...
def main(argv=None, config_files=None):
    ...
    CONF(args=argv[1:],
         project='sidserver',
         version=pbr.version.VersionInfo('sidserver').version_string(),
